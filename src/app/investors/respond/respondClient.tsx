...
export default function RespondClient() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const r = await fetch("/api/investors/info-request/validate", {
        method: "POST",
  ...
  async function submit() {
    ...
    const r = await fetch("/api/investors/info-request/submit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        token,
    ...
    setDone(true);
    setTimeout(() => router.push("/investors/status"), 1200);
  }
