...
export default function NdaClient() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const r = await fetch("/api/intelligence/nda/validate", {
        method: "POST",
      ...
      if ((j as any).ok && (j as any).used) {
        router.replace("/intelligence/nda-signed");
        return;
      }
  ...
  async function continueToSign() {
    ...
    const r = await fetch("/api/intelligence/nda/signwell/start", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        token,
    ...
    // Save token/name so /intelligence/nda-signed can finalize if needed
    try {
  ...
  if (!validate || (validate as any).ok === false) {
    ...
    return (
      <div className="max-w-2xl space-y-4 py-8">
        ...
        <a className="text-sm underline" href="/intelligence/status">Check application status</a>
      </div>
    );
  }
